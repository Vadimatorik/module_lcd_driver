...
uint8_t mono_lcd_lib_rle::get_byte ( void ) {
    ...
    if ( this->rle_loop != 0 ) {                // Если еще не закончились повторения.
        if (this->flag_rle == false) {
            this->rle_loop--;
            buffer = *( this->p - 1 );  // Если выдем loop раз значение справа от счетчика. Но мы уже приготовились смотреть на новый счетсчик, так что для нас он слева.
        } else {
        ...
        if (((int8_t)(*this->p)) > 0){		// > 0 - повторяющиеся, < 0 не повторяющиеся.
            flag_rle = false;
            buffer = *(this->p+1);			// Выдаем повторяющийся байт.
            this->rle_loop = *this->p - 1;		// Сохроняем колличество одинковых байт.
            this->p += 2;
        } else {						// Если байт не повторяющийся.
            this->flag_rle = true;
            this-> rle_loop = ((uint8_t)(abs((int8_t)(*this->p))))-1;
            this->p++;
