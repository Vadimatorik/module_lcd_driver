...
public:
    ...
    void    print_char_to_buffer ( const font_t& font, const uint8_t& x, const uint8_t& y, uint8_t ch, const uint8_t& color, const uint8_t& rle );
    void    print_string_to_buffer ( const font_t& font, uint8_t x, uint8_t y, char *string, uint8_t color, uint8_t rle);	// Целую строку.
    void    print_string_number_to_buffer ( const font_t& font, uint8_t x, uint8_t y, char *string, uint8_t number_char, uint8_t color, uint8_t rle);	// Рисуем заданное колличество символов на экране.
